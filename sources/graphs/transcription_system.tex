\begin{tikzpicture}
    ...
    \draw (-7.5, 0.0) node[layer] (midi_file) {MIDI File};
    \draw (-5.0, 0.0) node[layer] (note_list) {Note List};
    \draw (-2.5, 4.0) node[layer] (streams) {Streams};
    \draw (-2.5, 1.0) node[layer] (beats) {Beats};
    \draw (-2.5, 0.0) node[layer] (meter) {Meter};
    ...
    \draw (1.25, 0.0) node[layer] (quantized_notes) {Quantized \\ Notes};
    ...
    \draw[arrowstyle] (note_spelling.east) -- (score.south west);
    \draw[arrowstyle] (quantized_notes.east) -- (score.north west);
