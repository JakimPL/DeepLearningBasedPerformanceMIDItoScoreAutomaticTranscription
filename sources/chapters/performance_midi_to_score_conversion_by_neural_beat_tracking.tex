...
To reproduce results, we have followed the authors' training and evaluation setup, on the same train/validation/test dataset division. 

\subsubsection{Feature Preparation}

As the data comes from different sources, they need to be unified before training any model.

Initially, all MIDI files are gathered from datasets described in Section \ref{datasets}. According to authors, all pieces from the \texttt{ENSTDkCl} subset of the MAPS dataset are prepared for the test set. All other pieces are randomly assigned to the validation test, where $90\%$ of the data is training data\footnote{More precisely, pieces with id divisible by 10 are assigned to the validation set.}. See Table \ref{train_valid_test} for details.

A second step is to extract note sequence tensors and features from MIDI files and annotations in the case of the ASAP dataset. These features include all elements needed for the transcription: beats, downbeats, time signatures, key signatures, onsets, note values and hand part assignments.

After the preparation, the dataset is ready to be digested by models.

\subsubsection{Training}

For each trained model we set the maximum number of 50 epochs. Usually, models stabilized after 20 epochs. Training examples are augmented (refer to Section \ref{data_augmentation} for more information).

\section{Model Performance}

We reproduced the results using the training and evaluation setup provided by the authors. 

\subsection{$F_1$-measures}
